...
//			System.out.println("Sending back exception to client");
     ...
     * @return a new instance of the resolver.
     * @see NonblockingResolver
     * @exception UnknownHostException Failure occurred initializing
     * NonblockingResolvers
     */
     ...
     * received, or it is clear that there is no successful response. It is
     * possible that the QID of the Message returned will be different to the
     * QID of the Message which was sent. This is due to round-robin queries to
     * multiple servers, and timeout repeat queries - all of which need to be
     * sent with a unique QID. If your client code requires the same QID to be
     * returned as was sent, then you will need to use the NonblockingResolver
     * class instead, and perform your own retries/round-robins.
     *
     ...
     * times, registering a queue to receive a response on success or exception.
     * Multiple asynchronous lookups can be performed in parallel.
     *
     * @param query The query to send
     * @param queue The ResponseQueue to hold the response
     ...
     * @param index Return the resolver from the list of resolvers with the index provided.
     * @return the resolver at the given index.
     */
    public NonblockingResolver getResolver(int index) {
        if (index < resolvers.size()) {
            return (NonblockingResolver) resolvers.get(index);
        }
        return null;
    }
     ...
     *
     * @param flag If true, servers will be tried in round-robin order. If
     * false, servers will always be queried in the same order.
     */
